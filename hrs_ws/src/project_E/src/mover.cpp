...
#include "Project_E/srv/goCoord.h"
...
protected:
  ...
  // Service for walking
  ros::ServiceServer start_walking_srv;
...
public:
  ...
  */
  ...
  Nao_control(): my_actionClient(nh_, "blink", true) // Init the action client: target the /blink action server. Here "true" causes the action client to spin its own thread
  {
    ...
    start_walking_srv = nh_.advertiseService("/walk_to_position", &Nao_control::walkServiceCallback, this);
  ...
  void walkServiceCallback(const naoqi_bridge_msgs::WordRecognized::ConstPtr& msg)
  {
    //Save recognized words and their confidence level
   for (string word:msg->words){
     words_recognized.push_back(word);
