...
#include <project_E/BlinkAction.h>
...
public:
  ...
  */
  ...
  actionlib::SimpleActionClient<project_E::BlinkAction> my_actionClient;
  ...
  Nao_control(): my_actionClient(nh_, "blink", true) // Init the action client: target the /blink action server. Here "true" causes the action client to spin its own thread
  {
    ...
    walking_start_srv=nh_.serviceClient<std_srvs::Empty>("/needs_start_walk_pose_srv");
    ...
    std::vector<string> vocabulary = {"start organizing","stop","locate"};
  ...
  static void doneBlinkCallback(const actionlib::SimpleClientGoalState& state, const project_E::BlinkResultConstPtr& result)
  {
  ...
  static void feedbackBlinkCallback(const project_E::BlinkFeedbackConstPtr& feedback)
  {
  ...
  {
    //project_E::TactileGoal tactileGoal;
    std_srvs::Empty filler;
